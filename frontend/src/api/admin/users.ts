...
export interface SuperAdminSeat {
...
export async function changeLifecycle(
  ...
  reason: string
): Promise<{ user_id: number; status: 'active' | 'disabled' }> {
  const { data } = await apiClient.post<{ user_id: number; status: 'active' | 'disabled' }>(
...
export async function getSuperAdminSeat(): Promise<SuperAdminSeat> {
  const { data } = await apiClient.get<SuperAdminSeat>('/admin/governance/super-admin/seat')
  return data
}
...
export async function transferSuperAdmin(
  ...
  reason: string
): Promise<SuperAdminSeat> {
  const { data } = await apiClient.post<SuperAdminSeat>('/admin/governance/super-admin/transfer', {
    target_user_id: targetUserId,
    expected_seat_version: expectedSeatVersion,
    reason
  })
  return data
}
