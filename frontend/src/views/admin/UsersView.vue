<template>
  <AppLayout>
      ...
      <template #filters>
        <div class="flex flex-wrap items-center gap-3">
          <div
            ...
            class="flex w-full items-center justify-between gap-4 rounded-xl border border-amber-200 bg-amber-50/70 px-4 py-3 dark:border-amber-900/60 dark:bg-amber-950/20"
            data-test="super-admin-seat"
          >
            <div class="flex min-w-0 items-center gap-3">
              ...
              <div class="min-w-0">
                <p class="text-xs font-semibold uppercase tracking-wider text-amber-700 dark:text-amber-300">{{ t('admin.users.governance.seatLabel') }}</p>
                <p class="truncate text-sm text-gray-700 dark:text-gray-200">{{ authStore.user?.email }}</p>
              </div>
            </div>
            <span class="shrink-0 rounded-full bg-white/80 px-2.5 py-1 font-mono text-xs text-amber-800 ring-1 ring-amber-200 dark:bg-dark-800/80 dark:text-amber-200 dark:ring-amber-900">
              v{{ superAdminSeat?.seat_version ?? '—' }}
            </span>
          </div>
      ...
      <template #table>
        ...
        >
          ...
          <template #cell-role="{ value }">
            <span :class="['badge', value === 'super_admin' ? 'badge-warning' : value === 'admin' ? 'badge-purple' : 'badge-gray']">
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              ...
              <!-- Explicit lifecycle action; physical deletion is unavailable. -->
              <button
                v-if="canManageLifecycle(row)"
                @click="openGovernanceDialog(row, row.status === 'active' ? 'deactivate' : 'reactivate')"
                :class="[
        ...
        <div class="py-1">
          <template v-for="user in users" :key="user.id">
            <template v-if="user.id === activeMenuId">
              ...
              <!-- Only the current seat holder may transfer the single seat. -->
              <button
                v-if="canTransferSeat(user)"
                @click="openGovernanceDialog(user, 'transfer'); closeActionMenu()"
                class="flex w-full items-center gap-2 px-4 py-2 text-sm text-amber-700 hover:bg-amber-50 dark:text-amber-300 dark:hover:bg-amber-900/20"
                data-test="transfer-super-admin"
              >
                <Icon name="shield" size="sm" :stroke-width="2" />
                {{ t('admin.users.governance.transferAction') }}
              </button>
            </template>
          </template>
        </div>
    ...
    <UserGovernanceDialog
      :show="showGovernanceDialog"
      :user="governanceUser"
      :mode="governanceMode"
      :seat-version="superAdminSeat?.seat_version"
      @close="closeGovernanceDialog"
...
import type { PlatformQuotaItem, SuperAdminSeat } from '@/api/admin/users'
...
import UserGovernanceDialog from '@/components/admin/user/UserGovernanceDialog.vue'
...
const showGovernanceDialog = ref(false)
const showApiKeysModal = ref(false)
const showAttributesModal = ref(false)
const showPlatformQuotaModal = ref(false)
const editingUser = ref<AdminUser | null>(null)
const governanceUser = ref<AdminUser | null>(null)
const governanceMode = ref<'deactivate' | 'reactivate' | 'transfer'>('deactivate')
const superAdminSeat = ref<SuperAdminSeat | null>(null)
...
const canEditUser = (user: AdminUser) => user.role === 'user' || authStore.isSuperAdmin
const canManageLifecycle = (user: AdminUser) =>
  user.role === 'user' || (authStore.isSuperAdmin && user.role === 'admin')
const canTransferSeat = (user: AdminUser) =>
  authStore.isSuperAdmin && user.role === 'admin' && user.status === 'active'

const loadSuperAdminSeat = async () => {
  if (!authStore.isSuperAdmin) return
  try {
    superAdminSeat.value = await adminAPI.users.getSuperAdminSeat()
  } catch (error: any) {
    appStore.showError(error.response?.data?.message || error.response?.data?.detail || t('admin.users.governance.seatUnavailable'))
  }
}

const openGovernanceDialog = (user: AdminUser, mode: 'deactivate' | 'reactivate' | 'transfer') => {
  governanceUser.value = user
  governanceMode.value = mode
  showGovernanceDialog.value = true
}

const closeGovernanceDialog = () => {
  showGovernanceDialog.value = false
  governanceUser.value = null
}

const handleGovernanceSuccess = async () => {
  if (governanceMode.value === 'transfer') {
    await authStore.logout()
    window.location.assign('/login')
    return
  }
  await loadUsers()
  await loadSuperAdminSeat()
}
