...
type Client struct {
	...
	// AuditContentPart is the client for interacting with the AuditContentPart builders.
	AuditContentPart *AuditContentPartClient
	// AuditInteraction is the client for interacting with the AuditInteraction builders.
	AuditInteraction *AuditInteractionClient
	// AuthIdentity is the client for interacting with the AuthIdentity builders.
	AuthIdentity *AuthIdentityClient
	// AuthIdentityChannel is the client for interacting with the AuthIdentityChannel builders.
	AuthIdentityChannel *AuthIdentityChannelClient
	// AutoCandidatePool is the client for interacting with the AutoCandidatePool builders.
	AutoCandidatePool *AutoCandidatePoolClient
	...
	// GatewayInferenceRun is the client for interacting with the GatewayInferenceRun builders.
	GatewayInferenceRun *GatewayInferenceRunClient
	// GovernanceEvent is the client for interacting with the GovernanceEvent builders.
	GovernanceEvent *GovernanceEventClient
	...
	// RouteDecision is the client for interacting with the RouteDecision builders.
	RouteDecision *RouteDecisionClient
	...
	// SuperAdminSeat is the client for interacting with the SuperAdminSeat builders.
	SuperAdminSeat *SuperAdminSeatClient
...
func (c *Client) init() {
	...
	c.AuditContentPart = NewAuditContentPartClient(c.config)
	c.AuditInteraction = NewAuditInteractionClient(c.config)
	c.AuthIdentity = NewAuthIdentityClient(c.config)
	c.AuthIdentityChannel = NewAuthIdentityChannelClient(c.config)
	c.AutoCandidatePool = NewAutoCandidatePoolClient(c.config)
	...
	c.GatewayInferenceRun = NewGatewayInferenceRunClient(c.config)
	c.GovernanceEvent = NewGovernanceEventClient(c.config)
	...
	c.RouteDecision = NewRouteDecisionClient(c.config)
	c.SecuritySecret = NewSecuritySecretClient(c.config)
	c.Setting = NewSettingClient(c.config)
	c.SubscriptionPlan = NewSubscriptionPlanClient(c.config)
	c.SuperAdminSeat = NewSuperAdminSeatClient(c.config)
...
func (c *Client) Tx(ctx context.Context) (*Tx, error) {
	...
	return &Tx{
		...
		AuditContentPart:              NewAuditContentPartClient(cfg),
		AuditInteraction:              NewAuditInteractionClient(cfg),
		AuthIdentity:                  NewAuthIdentityClient(cfg),
		AuthIdentityChannel:           NewAuthIdentityChannelClient(cfg),
		AutoCandidatePool:             NewAutoCandidatePoolClient(cfg),
		...
		GatewayInferenceRun:           NewGatewayInferenceRunClient(cfg),
		GovernanceEvent:               NewGovernanceEventClient(cfg),
		...
		RouteDecision:                 NewRouteDecisionClient(cfg),
		SecuritySecret:                NewSecuritySecretClient(cfg),
		Setting:                       NewSettingClient(cfg),
		SubscriptionPlan:              NewSubscriptionPlanClient(cfg),
		SuperAdminSeat:                NewSuperAdminSeatClient(cfg),
...
func (c *Client) Use(hooks ...Hook) {
	for _, n := range []interface{ Use(...Hook) }{
		...
		c.AuditContentPart, c.AuditInteraction, c.AuthIdentity, c.AuthIdentityChannel,
		c.AutoCandidatePool, c.BatchImageEvent, c.BatchImageItem, c.BatchImageJob,
		c.ChannelMonitor, c.ChannelMonitorDailyRollup, c.ChannelMonitorHistory,
		c.ChannelMonitorRequestTemplate, c.ErrorPassthroughRule, c.GatewayInferenceRun,
		c.GovernanceEvent, c.Group, c.IdempotencyRecord, c.IdentityAdoptionDecision,
		c.ModelCatalogEntry, c.PaymentAuditLog, c.PaymentOrder,
		c.PaymentProviderInstance, c.PendingAuthSession, c.PromoCode, c.PromoCodeUsage,
		c.Proxy, c.RedeemCode, c.RouteDecision, c.SecuritySecret, c.Setting,
		c.SubscriptionPlan, c.SuperAdminSeat, c.TLSFingerprintProfile,
		c.UsageCleanupTask, c.UsageLog, c.User, c.UserAllowedGroup,
		c.UserAttributeDefinition, c.UserAttributeValue, c.UserPlatformQuota,
		c.UserSubscription, c.WorkSession,
	} {
		n.Use(hooks...)
	}
}
...
func (c *Client) Mutate(ctx context.Context, m Mutation) (Value, error) {
	...
	case *AuditContentPartMutation:
		return c.AuditContentPart.mutate(ctx, m)
	case *AuditInteractionMutation:
		return c.AuditInteraction.mutate(ctx, m)
	...
	case *AutoCandidatePoolMutation:
		return c.AutoCandidatePool.mutate(ctx, m)
	...
	case *GatewayInferenceRunMutation:
		return c.GatewayInferenceRun.mutate(ctx, m)
	case *GovernanceEventMutation:
		return c.GovernanceEvent.mutate(ctx, m)
	case *GroupMutation:
	...
	case *RouteDecisionMutation:
		return c.RouteDecision.mutate(ctx, m)
	...
	case *SuperAdminSeatMutation:
		return c.SuperAdminSeat.mutate(ctx, m)
...
// AuditContentPartClient is a client for the AuditContentPart schema.
type AuditContentPartClient struct {
	config
}

// NewAuditContentPartClient returns a client for the AuditContentPart from the given config.
func NewAuditContentPartClient(c config) *AuditContentPartClient {
	return &AuditContentPartClient{config: c}
}

// Use adds a list of mutation hooks to the hooks stack.
// A call to `Use(f, g, h)` equals to `auditcontentpart.Hooks(f(g(h())))`.
func (c *AuditContentPartClient) Use(hooks ...Hook) {
	c.hooks.AuditContentPart = append(c.hooks.AuditContentPart, hooks...)
}

// Intercept adds a list of query interceptors to the interceptors stack.
// A call to `Intercept(f, g, h)` equals to `auditcontentpart.Intercept(f(g(h())))`.
func (c *AuditContentPartClient) Intercept(interceptors ...Interceptor) {
	c.inters.AuditContentPart = append(c.inters.AuditContentPart, interceptors...)
}

// Create returns a builder for creating a AuditContentPart entity.
func (c *AuditContentPartClient) Create() *AuditContentPartCreate {
	mutation := newAuditContentPartMutation(c.config, OpCreate)
	return &AuditContentPartCreate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// CreateBulk returns a builder for creating a bulk of AuditContentPart entities.
func (c *AuditContentPartClient) CreateBulk(builders ...*AuditContentPartCreate) *AuditContentPartCreateBulk {
	return &AuditContentPartCreateBulk{config: c.config, builders: builders}
}

// MapCreateBulk creates a bulk creation builder from the given slice. For each item in the slice, the function creates
// a builder and applies setFunc on it.
func (c *AuditContentPartClient) MapCreateBulk(slice any, setFunc func(*AuditContentPartCreate, int)) *AuditContentPartCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &AuditContentPartCreateBulk{err: fmt.Errorf("calling to AuditContentPartClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*AuditContentPartCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
		setFunc(builders[i], i)
	}
...
// Update returns an update builder for AuditContentPart.
func (c *AuditContentPartClient) Update() *AuditContentPartUpdate {
	mutation := newAuditContentPartMutation(c.config, OpUpdate)
	return &AuditContentPartUpdate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOne returns an update builder for the given entity.
func (c *AuditContentPartClient) UpdateOne(_m *AuditContentPart) *AuditContentPartUpdateOne {
	mutation := newAuditContentPartMutation(c.config, OpUpdateOne, withAuditContentPart(_m))
	return &AuditContentPartUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *AuditContentPartClient) UpdateOneID(id uuid.UUID) *AuditContentPartUpdateOne {
	mutation := newAuditContentPartMutation(c.config, OpUpdateOne, withAuditContentPartID(id))
	return &AuditContentPartUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// Delete returns a delete builder for AuditContentPart.
func (c *AuditContentPartClient) Delete() *AuditContentPartDelete {
	mutation := newAuditContentPartMutation(c.config, OpDelete)
	return &AuditContentPartDelete{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// DeleteOne returns a builder for deleting the given entity.
func (c *AuditContentPartClient) DeleteOne(_m *AuditContentPart) *AuditContentPartDeleteOne {
	return c.DeleteOneID(_m.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *AuditContentPartClient) DeleteOneID(id uuid.UUID) *AuditContentPartDeleteOne {
	builder := c.Delete().Where(auditcontentpart.ID(id))
	builder.mutation.id = &id
	builder.mutation.op = OpDeleteOne
	return &AuditContentPartDeleteOne{builder}
}

// Query returns a query builder for AuditContentPart.
func (c *AuditContentPartClient) Query() *AuditContentPartQuery {
	return &AuditContentPartQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeAuditContentPart},
		inters: c.Interceptors(),
	}
}

// Get returns a AuditContentPart entity by its id.
func (c *AuditContentPartClient) Get(ctx context.Context, id uuid.UUID) (*AuditContentPart, error) {
	return c.Query().Where(auditcontentpart.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *AuditContentPartClient) GetX(ctx context.Context, id uuid.UUID) *AuditContentPart {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// QueryInteraction queries the interaction edge of a AuditContentPart.
func (c *AuditContentPartClient) QueryInteraction(_m *AuditContentPart) *AuditInteractionQuery {
	query := (&AuditInteractionClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(auditcontentpart.Table, auditcontentpart.FieldID, id),
...
// Hooks returns the client hooks.
func (c *AuditContentPartClient) Hooks() []Hook {
	return c.hooks.AuditContentPart
}

// Interceptors returns the client interceptors.
func (c *AuditContentPartClient) Interceptors() []Interceptor {
	return c.inters.AuditContentPart
}

func (c *AuditContentPartClient) mutate(ctx context.Context, m *AuditContentPartMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&AuditContentPartCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&AuditContentPartUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&AuditContentPartUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&AuditContentPartDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown AuditContentPart mutation op: %q", m.Op())
	}
}

// AuditInteractionClient is a client for the AuditInteraction schema.
type AuditInteractionClient struct {
	config
}

// NewAuditInteractionClient returns a client for the AuditInteraction from the given config.
func NewAuditInteractionClient(c config) *AuditInteractionClient {
	return &AuditInteractionClient{config: c}
}

// Use adds a list of mutation hooks to the hooks stack.
// A call to `Use(f, g, h)` equals to `auditinteraction.Hooks(f(g(h())))`.
func (c *AuditInteractionClient) Use(hooks ...Hook) {
	c.hooks.AuditInteraction = append(c.hooks.AuditInteraction, hooks...)
}

// Intercept adds a list of query interceptors to the interceptors stack.
// A call to `Intercept(f, g, h)` equals to `auditinteraction.Intercept(f(g(h())))`.
func (c *AuditInteractionClient) Intercept(interceptors ...Interceptor) {
	c.inters.AuditInteraction = append(c.inters.AuditInteraction, interceptors...)
}

// Create returns a builder for creating a AuditInteraction entity.
func (c *AuditInteractionClient) Create() *AuditInteractionCreate {
	mutation := newAuditInteractionMutation(c.config, OpCreate)
	return &AuditInteractionCreate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// CreateBulk returns a builder for creating a bulk of AuditInteraction entities.
func (c *AuditInteractionClient) CreateBulk(builders ...*AuditInteractionCreate) *AuditInteractionCreateBulk {
	return &AuditInteractionCreateBulk{config: c.config, builders: builders}
}

// MapCreateBulk creates a bulk creation builder from the given slice. For each item in the slice, the function creates
// a builder and applies setFunc on it.
func (c *AuditInteractionClient) MapCreateBulk(slice any, setFunc func(*AuditInteractionCreate, int)) *AuditInteractionCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &AuditInteractionCreateBulk{err: fmt.Errorf("calling to AuditInteractionClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*AuditInteractionCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
		setFunc(builders[i], i)
	}
...
// Update returns an update builder for AuditInteraction.
func (c *AuditInteractionClient) Update() *AuditInteractionUpdate {
	mutation := newAuditInteractionMutation(c.config, OpUpdate)
	return &AuditInteractionUpdate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOne returns an update builder for the given entity.
func (c *AuditInteractionClient) UpdateOne(_m *AuditInteraction) *AuditInteractionUpdateOne {
	mutation := newAuditInteractionMutation(c.config, OpUpdateOne, withAuditInteraction(_m))
	return &AuditInteractionUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *AuditInteractionClient) UpdateOneID(id uuid.UUID) *AuditInteractionUpdateOne {
	mutation := newAuditInteractionMutation(c.config, OpUpdateOne, withAuditInteractionID(id))
	return &AuditInteractionUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// Delete returns a delete builder for AuditInteraction.
func (c *AuditInteractionClient) Delete() *AuditInteractionDelete {
	mutation := newAuditInteractionMutation(c.config, OpDelete)
	return &AuditInteractionDelete{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// DeleteOne returns a builder for deleting the given entity.
func (c *AuditInteractionClient) DeleteOne(_m *AuditInteraction) *AuditInteractionDeleteOne {
	return c.DeleteOneID(_m.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *AuditInteractionClient) DeleteOneID(id uuid.UUID) *AuditInteractionDeleteOne {
	builder := c.Delete().Where(auditinteraction.ID(id))
	builder.mutation.id = &id
	builder.mutation.op = OpDeleteOne
	return &AuditInteractionDeleteOne{builder}
}

// Query returns a query builder for AuditInteraction.
func (c *AuditInteractionClient) Query() *AuditInteractionQuery {
	return &AuditInteractionQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeAuditInteraction},
		inters: c.Interceptors(),
	}
}

// Get returns a AuditInteraction entity by its id.
func (c *AuditInteractionClient) Get(ctx context.Context, id uuid.UUID) (*AuditInteraction, error) {
	return c.Query().Where(auditinteraction.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *AuditInteractionClient) GetX(ctx context.Context, id uuid.UUID) *AuditInteraction {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// QueryContentParts queries the content_parts edge of a AuditInteraction.
func (c *AuditInteractionClient) QueryContentParts(_m *AuditInteraction) *AuditContentPartQuery {
	query := (&AuditContentPartClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(auditinteraction.Table, auditinteraction.FieldID, id),
...
// Hooks returns the client hooks.
func (c *AuditInteractionClient) Hooks() []Hook {
	return c.hooks.AuditInteraction
}

// Interceptors returns the client interceptors.
func (c *AuditInteractionClient) Interceptors() []Interceptor {
	return c.inters.AuditInteraction
}

func (c *AuditInteractionClient) mutate(ctx context.Context, m *AuditInteractionMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&AuditInteractionCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&AuditInteractionUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&AuditInteractionUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&AuditInteractionDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown AuditInteraction mutation op: %q", m.Op())
	}
}
...
// AutoCandidatePoolClient is a client for the AutoCandidatePool schema.
type AutoCandidatePoolClient struct {
	config
}

// NewAutoCandidatePoolClient returns a client for the AutoCandidatePool from the given config.
func NewAutoCandidatePoolClient(c config) *AutoCandidatePoolClient {
	return &AutoCandidatePoolClient{config: c}
}

// Use adds a list of mutation hooks to the hooks stack.
// A call to `Use(f, g, h)` equals to `autocandidatepool.Hooks(f(g(h())))`.
func (c *AutoCandidatePoolClient) Use(hooks ...Hook) {
	c.hooks.AutoCandidatePool = append(c.hooks.AutoCandidatePool, hooks...)
}

// Intercept adds a list of query interceptors to the interceptors stack.
// A call to `Intercept(f, g, h)` equals to `autocandidatepool.Intercept(f(g(h())))`.
func (c *AutoCandidatePoolClient) Intercept(interceptors ...Interceptor) {
	c.inters.AutoCandidatePool = append(c.inters.AutoCandidatePool, interceptors...)
}

// Create returns a builder for creating a AutoCandidatePool entity.
func (c *AutoCandidatePoolClient) Create() *AutoCandidatePoolCreate {
	mutation := newAutoCandidatePoolMutation(c.config, OpCreate)
	return &AutoCandidatePoolCreate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// CreateBulk returns a builder for creating a bulk of AutoCandidatePool entities.
func (c *AutoCandidatePoolClient) CreateBulk(builders ...*AutoCandidatePoolCreate) *AutoCandidatePoolCreateBulk {
	return &AutoCandidatePoolCreateBulk{config: c.config, builders: builders}
}

// MapCreateBulk creates a bulk creation builder from the given slice. For each item in the slice, the function creates
// a builder and applies setFunc on it.
func (c *AutoCandidatePoolClient) MapCreateBulk(slice any, setFunc func(*AutoCandidatePoolCreate, int)) *AutoCandidatePoolCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &AutoCandidatePoolCreateBulk{err: fmt.Errorf("calling to AutoCandidatePoolClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*AutoCandidatePoolCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
		setFunc(builders[i], i)
	}
...
// Update returns an update builder for AutoCandidatePool.
func (c *AutoCandidatePoolClient) Update() *AutoCandidatePoolUpdate {
	mutation := newAutoCandidatePoolMutation(c.config, OpUpdate)
	return &AutoCandidatePoolUpdate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOne returns an update builder for the given entity.
func (c *AutoCandidatePoolClient) UpdateOne(_m *AutoCandidatePool) *AutoCandidatePoolUpdateOne {
	mutation := newAutoCandidatePoolMutation(c.config, OpUpdateOne, withAutoCandidatePool(_m))
	return &AutoCandidatePoolUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *AutoCandidatePoolClient) UpdateOneID(id uuid.UUID) *AutoCandidatePoolUpdateOne {
	mutation := newAutoCandidatePoolMutation(c.config, OpUpdateOne, withAutoCandidatePoolID(id))
	return &AutoCandidatePoolUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// Delete returns a delete builder for AutoCandidatePool.
func (c *AutoCandidatePoolClient) Delete() *AutoCandidatePoolDelete {
	mutation := newAutoCandidatePoolMutation(c.config, OpDelete)
	return &AutoCandidatePoolDelete{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// DeleteOne returns a builder for deleting the given entity.
func (c *AutoCandidatePoolClient) DeleteOne(_m *AutoCandidatePool) *AutoCandidatePoolDeleteOne {
	return c.DeleteOneID(_m.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *AutoCandidatePoolClient) DeleteOneID(id uuid.UUID) *AutoCandidatePoolDeleteOne {
	builder := c.Delete().Where(autocandidatepool.ID(id))
	builder.mutation.id = &id
	builder.mutation.op = OpDeleteOne
	return &AutoCandidatePoolDeleteOne{builder}
}

// Query returns a query builder for AutoCandidatePool.
func (c *AutoCandidatePoolClient) Query() *AutoCandidatePoolQuery {
	return &AutoCandidatePoolQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeAutoCandidatePool},
		inters: c.Interceptors(),
	}
}

// Get returns a AutoCandidatePool entity by its id.
func (c *AutoCandidatePoolClient) Get(ctx context.Context, id uuid.UUID) (*AutoCandidatePool, error) {
	return c.Query().Where(autocandidatepool.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *AutoCandidatePoolClient) GetX(ctx context.Context, id uuid.UUID) *AutoCandidatePool {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// Hooks returns the client hooks.
func (c *AutoCandidatePoolClient) Hooks() []Hook {
	return c.hooks.AutoCandidatePool
}

// Interceptors returns the client interceptors.
func (c *AutoCandidatePoolClient) Interceptors() []Interceptor {
	return c.inters.AutoCandidatePool
}

func (c *AutoCandidatePoolClient) mutate(ctx context.Context, m *AutoCandidatePoolMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&AutoCandidatePoolCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&AutoCandidatePoolUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&AutoCandidatePoolUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&AutoCandidatePoolDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown AutoCandidatePool mutation op: %q", m.Op())
	}
}
...
// GatewayInferenceRunClient is a client for the GatewayInferenceRun schema.
type GatewayInferenceRunClient struct {
	config
}

// NewGatewayInferenceRunClient returns a client for the GatewayInferenceRun from the given config.
func NewGatewayInferenceRunClient(c config) *GatewayInferenceRunClient {
	return &GatewayInferenceRunClient{config: c}
}

// Use adds a list of mutation hooks to the hooks stack.
// A call to `Use(f, g, h)` equals to `gatewayinferencerun.Hooks(f(g(h())))`.
func (c *GatewayInferenceRunClient) Use(hooks ...Hook) {
	c.hooks.GatewayInferenceRun = append(c.hooks.GatewayInferenceRun, hooks...)
}

// Intercept adds a list of query interceptors to the interceptors stack.
// A call to `Intercept(f, g, h)` equals to `gatewayinferencerun.Intercept(f(g(h())))`.
func (c *GatewayInferenceRunClient) Intercept(interceptors ...Interceptor) {
	c.inters.GatewayInferenceRun = append(c.inters.GatewayInferenceRun, interceptors...)
}

// Create returns a builder for creating a GatewayInferenceRun entity.
func (c *GatewayInferenceRunClient) Create() *GatewayInferenceRunCreate {
	mutation := newGatewayInferenceRunMutation(c.config, OpCreate)
	return &GatewayInferenceRunCreate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// CreateBulk returns a builder for creating a bulk of GatewayInferenceRun entities.
func (c *GatewayInferenceRunClient) CreateBulk(builders ...*GatewayInferenceRunCreate) *GatewayInferenceRunCreateBulk {
	return &GatewayInferenceRunCreateBulk{config: c.config, builders: builders}
}

// MapCreateBulk creates a bulk creation builder from the given slice. For each item in the slice, the function creates
// a builder and applies setFunc on it.
func (c *GatewayInferenceRunClient) MapCreateBulk(slice any, setFunc func(*GatewayInferenceRunCreate, int)) *GatewayInferenceRunCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &GatewayInferenceRunCreateBulk{err: fmt.Errorf("calling to GatewayInferenceRunClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*GatewayInferenceRunCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
		setFunc(builders[i], i)
	}
...
// Update returns an update builder for GatewayInferenceRun.
func (c *GatewayInferenceRunClient) Update() *GatewayInferenceRunUpdate {
	mutation := newGatewayInferenceRunMutation(c.config, OpUpdate)
	return &GatewayInferenceRunUpdate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOne returns an update builder for the given entity.
func (c *GatewayInferenceRunClient) UpdateOne(_m *GatewayInferenceRun) *GatewayInferenceRunUpdateOne {
	mutation := newGatewayInferenceRunMutation(c.config, OpUpdateOne, withGatewayInferenceRun(_m))
	return &GatewayInferenceRunUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *GatewayInferenceRunClient) UpdateOneID(id uuid.UUID) *GatewayInferenceRunUpdateOne {
	mutation := newGatewayInferenceRunMutation(c.config, OpUpdateOne, withGatewayInferenceRunID(id))
	return &GatewayInferenceRunUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// Delete returns a delete builder for GatewayInferenceRun.
func (c *GatewayInferenceRunClient) Delete() *GatewayInferenceRunDelete {
	mutation := newGatewayInferenceRunMutation(c.config, OpDelete)
	return &GatewayInferenceRunDelete{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// DeleteOne returns a builder for deleting the given entity.
func (c *GatewayInferenceRunClient) DeleteOne(_m *GatewayInferenceRun) *GatewayInferenceRunDeleteOne {
	return c.DeleteOneID(_m.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *GatewayInferenceRunClient) DeleteOneID(id uuid.UUID) *GatewayInferenceRunDeleteOne {
	builder := c.Delete().Where(gatewayinferencerun.ID(id))
	builder.mutation.id = &id
	builder.mutation.op = OpDeleteOne
	return &GatewayInferenceRunDeleteOne{builder}
}

// Query returns a query builder for GatewayInferenceRun.
func (c *GatewayInferenceRunClient) Query() *GatewayInferenceRunQuery {
	return &GatewayInferenceRunQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeGatewayInferenceRun},
		inters: c.Interceptors(),
	}
}

// Get returns a GatewayInferenceRun entity by its id.
func (c *GatewayInferenceRunClient) Get(ctx context.Context, id uuid.UUID) (*GatewayInferenceRun, error) {
	return c.Query().Where(gatewayinferencerun.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *GatewayInferenceRunClient) GetX(ctx context.Context, id uuid.UUID) *GatewayInferenceRun {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// Hooks returns the client hooks.
func (c *GatewayInferenceRunClient) Hooks() []Hook {
	return c.hooks.GatewayInferenceRun
}

// Interceptors returns the client interceptors.
func (c *GatewayInferenceRunClient) Interceptors() []Interceptor {
	return c.inters.GatewayInferenceRun
}

func (c *GatewayInferenceRunClient) mutate(ctx context.Context, m *GatewayInferenceRunMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&GatewayInferenceRunCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&GatewayInferenceRunUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&GatewayInferenceRunUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&GatewayInferenceRunDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown GatewayInferenceRun mutation op: %q", m.Op())
	}
}

// GovernanceEventClient is a client for the GovernanceEvent schema.
type GovernanceEventClient struct {
	config
}

// NewGovernanceEventClient returns a client for the GovernanceEvent from the given config.
func NewGovernanceEventClient(c config) *GovernanceEventClient {
	return &GovernanceEventClient{config: c}
}

// Use adds a list of mutation hooks to the hooks stack.
// A call to `Use(f, g, h)` equals to `governanceevent.Hooks(f(g(h())))`.
func (c *GovernanceEventClient) Use(hooks ...Hook) {
	c.hooks.GovernanceEvent = append(c.hooks.GovernanceEvent, hooks...)
}

// Intercept adds a list of query interceptors to the interceptors stack.
// A call to `Intercept(f, g, h)` equals to `governanceevent.Intercept(f(g(h())))`.
func (c *GovernanceEventClient) Intercept(interceptors ...Interceptor) {
	c.inters.GovernanceEvent = append(c.inters.GovernanceEvent, interceptors...)
}

// Create returns a builder for creating a GovernanceEvent entity.
func (c *GovernanceEventClient) Create() *GovernanceEventCreate {
	mutation := newGovernanceEventMutation(c.config, OpCreate)
	return &GovernanceEventCreate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// CreateBulk returns a builder for creating a bulk of GovernanceEvent entities.
func (c *GovernanceEventClient) CreateBulk(builders ...*GovernanceEventCreate) *GovernanceEventCreateBulk {
	return &GovernanceEventCreateBulk{config: c.config, builders: builders}
}

// MapCreateBulk creates a bulk creation builder from the given slice. For each item in the slice, the function creates
// a builder and applies setFunc on it.
func (c *GovernanceEventClient) MapCreateBulk(slice any, setFunc func(*GovernanceEventCreate, int)) *GovernanceEventCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &GovernanceEventCreateBulk{err: fmt.Errorf("calling to GovernanceEventClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*GovernanceEventCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
		setFunc(builders[i], i)
	}
...
// Update returns an update builder for GovernanceEvent.
func (c *GovernanceEventClient) Update() *GovernanceEventUpdate {
	mutation := newGovernanceEventMutation(c.config, OpUpdate)
	return &GovernanceEventUpdate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOne returns an update builder for the given entity.
func (c *GovernanceEventClient) UpdateOne(_m *GovernanceEvent) *GovernanceEventUpdateOne {
	mutation := newGovernanceEventMutation(c.config, OpUpdateOne, withGovernanceEvent(_m))
	return &GovernanceEventUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *GovernanceEventClient) UpdateOneID(id uuid.UUID) *GovernanceEventUpdateOne {
	mutation := newGovernanceEventMutation(c.config, OpUpdateOne, withGovernanceEventID(id))
	return &GovernanceEventUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// Delete returns a delete builder for GovernanceEvent.
func (c *GovernanceEventClient) Delete() *GovernanceEventDelete {
	mutation := newGovernanceEventMutation(c.config, OpDelete)
	return &GovernanceEventDelete{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// DeleteOne returns a builder for deleting the given entity.
func (c *GovernanceEventClient) DeleteOne(_m *GovernanceEvent) *GovernanceEventDeleteOne {
	return c.DeleteOneID(_m.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *GovernanceEventClient) DeleteOneID(id uuid.UUID) *GovernanceEventDeleteOne {
	builder := c.Delete().Where(governanceevent.ID(id))
	builder.mutation.id = &id
	builder.mutation.op = OpDeleteOne
	return &GovernanceEventDeleteOne{builder}
}

// Query returns a query builder for GovernanceEvent.
func (c *GovernanceEventClient) Query() *GovernanceEventQuery {
	return &GovernanceEventQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeGovernanceEvent},
		inters: c.Interceptors(),
	}
}

// Get returns a GovernanceEvent entity by its id.
func (c *GovernanceEventClient) Get(ctx context.Context, id uuid.UUID) (*GovernanceEvent, error) {
	return c.Query().Where(governanceevent.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *GovernanceEventClient) GetX(ctx context.Context, id uuid.UUID) *GovernanceEvent {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// QueryActor queries the actor edge of a GovernanceEvent.
func (c *GovernanceEventClient) QueryActor(_m *GovernanceEvent) *UserQuery {
	query := (&UserClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(governanceevent.Table, governanceevent.FieldID, id),
			sqlgraph.To(user.Table, user.FieldID),
			sqlgraph.Edge(sqlgraph.M2O, false, governanceevent.ActorTable, governanceevent.ActorColumn),
		)
		fromV = sqlgraph.Neighbors(_m.driver.Dialect(), step)
		return fromV, nil
	}
...
// Hooks returns the client hooks.
func (c *GovernanceEventClient) Hooks() []Hook {
	return c.hooks.GovernanceEvent
}

// Interceptors returns the client interceptors.
func (c *GovernanceEventClient) Interceptors() []Interceptor {
	return c.inters.GovernanceEvent
}

func (c *GovernanceEventClient) mutate(ctx context.Context, m *GovernanceEventMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&GovernanceEventCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&GovernanceEventUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&GovernanceEventUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&GovernanceEventDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown GovernanceEvent mutation op: %q", m.Op())
	}
}
...
// UpdateOneID returns an update builder for the given id.
func (c *ModelCatalogEntryClient) UpdateOneID(id uuid.UUID) *ModelCatalogEntryUpdateOne {
...
// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *ModelCatalogEntryClient) DeleteOneID(id uuid.UUID) *ModelCatalogEntryDeleteOne {
...
func (c *ModelCatalogEntryClient) Get(ctx context.Context, id uuid.UUID) (*ModelCatalogEntry, error) {
	return c.Query().Where(modelcatalogentry.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *ModelCatalogEntryClient) GetX(ctx context.Context, id uuid.UUID) *ModelCatalogEntry {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// RouteDecisionClient is a client for the RouteDecision schema.
type RouteDecisionClient struct {
	config
}

// NewRouteDecisionClient returns a client for the RouteDecision from the given config.
func NewRouteDecisionClient(c config) *RouteDecisionClient {
	return &RouteDecisionClient{config: c}
}

// Use adds a list of mutation hooks to the hooks stack.
// A call to `Use(f, g, h)` equals to `routedecision.Hooks(f(g(h())))`.
func (c *RouteDecisionClient) Use(hooks ...Hook) {
	c.hooks.RouteDecision = append(c.hooks.RouteDecision, hooks...)
}

// Intercept adds a list of query interceptors to the interceptors stack.
// A call to `Intercept(f, g, h)` equals to `routedecision.Intercept(f(g(h())))`.
func (c *RouteDecisionClient) Intercept(interceptors ...Interceptor) {
	c.inters.RouteDecision = append(c.inters.RouteDecision, interceptors...)
}

// Create returns a builder for creating a RouteDecision entity.
func (c *RouteDecisionClient) Create() *RouteDecisionCreate {
	mutation := newRouteDecisionMutation(c.config, OpCreate)
	return &RouteDecisionCreate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// CreateBulk returns a builder for creating a bulk of RouteDecision entities.
func (c *RouteDecisionClient) CreateBulk(builders ...*RouteDecisionCreate) *RouteDecisionCreateBulk {
	return &RouteDecisionCreateBulk{config: c.config, builders: builders}
}

// MapCreateBulk creates a bulk creation builder from the given slice. For each item in the slice, the function creates
// a builder and applies setFunc on it.
func (c *RouteDecisionClient) MapCreateBulk(slice any, setFunc func(*RouteDecisionCreate, int)) *RouteDecisionCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &RouteDecisionCreateBulk{err: fmt.Errorf("calling to RouteDecisionClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*RouteDecisionCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
		setFunc(builders[i], i)
	}
...
// Update returns an update builder for RouteDecision.
func (c *RouteDecisionClient) Update() *RouteDecisionUpdate {
	mutation := newRouteDecisionMutation(c.config, OpUpdate)
	return &RouteDecisionUpdate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOne returns an update builder for the given entity.
func (c *RouteDecisionClient) UpdateOne(_m *RouteDecision) *RouteDecisionUpdateOne {
	mutation := newRouteDecisionMutation(c.config, OpUpdateOne, withRouteDecision(_m))
	return &RouteDecisionUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *RouteDecisionClient) UpdateOneID(id uuid.UUID) *RouteDecisionUpdateOne {
	mutation := newRouteDecisionMutation(c.config, OpUpdateOne, withRouteDecisionID(id))
	return &RouteDecisionUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// Delete returns a delete builder for RouteDecision.
func (c *RouteDecisionClient) Delete() *RouteDecisionDelete {
	mutation := newRouteDecisionMutation(c.config, OpDelete)
	return &RouteDecisionDelete{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// DeleteOne returns a builder for deleting the given entity.
func (c *RouteDecisionClient) DeleteOne(_m *RouteDecision) *RouteDecisionDeleteOne {
	return c.DeleteOneID(_m.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *RouteDecisionClient) DeleteOneID(id uuid.UUID) *RouteDecisionDeleteOne {
	builder := c.Delete().Where(routedecision.ID(id))
	builder.mutation.id = &id
	builder.mutation.op = OpDeleteOne
	return &RouteDecisionDeleteOne{builder}
}

// Query returns a query builder for RouteDecision.
func (c *RouteDecisionClient) Query() *RouteDecisionQuery {
	return &RouteDecisionQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeRouteDecision},
		inters: c.Interceptors(),
	}
}

// Get returns a RouteDecision entity by its id.
func (c *RouteDecisionClient) Get(ctx context.Context, id uuid.UUID) (*RouteDecision, error) {
	return c.Query().Where(routedecision.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *RouteDecisionClient) GetX(ctx context.Context, id uuid.UUID) *RouteDecision {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// Hooks returns the client hooks.
func (c *RouteDecisionClient) Hooks() []Hook {
	return c.hooks.RouteDecision
}

// Interceptors returns the client interceptors.
func (c *RouteDecisionClient) Interceptors() []Interceptor {
	return c.inters.RouteDecision
}

func (c *RouteDecisionClient) mutate(ctx context.Context, m *RouteDecisionMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&RouteDecisionCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&RouteDecisionUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&RouteDecisionUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&RouteDecisionDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown RouteDecision mutation op: %q", m.Op())
	}
}
...
// SuperAdminSeatClient is a client for the SuperAdminSeat schema.
type SuperAdminSeatClient struct {
	config
}

// NewSuperAdminSeatClient returns a client for the SuperAdminSeat from the given config.
func NewSuperAdminSeatClient(c config) *SuperAdminSeatClient {
	return &SuperAdminSeatClient{config: c}
}

// Use adds a list of mutation hooks to the hooks stack.
// A call to `Use(f, g, h)` equals to `superadminseat.Hooks(f(g(h())))`.
func (c *SuperAdminSeatClient) Use(hooks ...Hook) {
	c.hooks.SuperAdminSeat = append(c.hooks.SuperAdminSeat, hooks...)
}

// Intercept adds a list of query interceptors to the interceptors stack.
// A call to `Intercept(f, g, h)` equals to `superadminseat.Intercept(f(g(h())))`.
func (c *SuperAdminSeatClient) Intercept(interceptors ...Interceptor) {
	c.inters.SuperAdminSeat = append(c.inters.SuperAdminSeat, interceptors...)
}

// Create returns a builder for creating a SuperAdminSeat entity.
func (c *SuperAdminSeatClient) Create() *SuperAdminSeatCreate {
	mutation := newSuperAdminSeatMutation(c.config, OpCreate)
	return &SuperAdminSeatCreate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// CreateBulk returns a builder for creating a bulk of SuperAdminSeat entities.
func (c *SuperAdminSeatClient) CreateBulk(builders ...*SuperAdminSeatCreate) *SuperAdminSeatCreateBulk {
	return &SuperAdminSeatCreateBulk{config: c.config, builders: builders}
}

// MapCreateBulk creates a bulk creation builder from the given slice. For each item in the slice, the function creates
// a builder and applies setFunc on it.
func (c *SuperAdminSeatClient) MapCreateBulk(slice any, setFunc func(*SuperAdminSeatCreate, int)) *SuperAdminSeatCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &SuperAdminSeatCreateBulk{err: fmt.Errorf("calling to SuperAdminSeatClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*SuperAdminSeatCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
		setFunc(builders[i], i)
	}
...
// Update returns an update builder for SuperAdminSeat.
func (c *SuperAdminSeatClient) Update() *SuperAdminSeatUpdate {
	mutation := newSuperAdminSeatMutation(c.config, OpUpdate)
	return &SuperAdminSeatUpdate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOne returns an update builder for the given entity.
func (c *SuperAdminSeatClient) UpdateOne(_m *SuperAdminSeat) *SuperAdminSeatUpdateOne {
	mutation := newSuperAdminSeatMutation(c.config, OpUpdateOne, withSuperAdminSeat(_m))
	return &SuperAdminSeatUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *SuperAdminSeatClient) UpdateOneID(id int16) *SuperAdminSeatUpdateOne {
	mutation := newSuperAdminSeatMutation(c.config, OpUpdateOne, withSuperAdminSeatID(id))
	return &SuperAdminSeatUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// Delete returns a delete builder for SuperAdminSeat.
func (c *SuperAdminSeatClient) Delete() *SuperAdminSeatDelete {
	mutation := newSuperAdminSeatMutation(c.config, OpDelete)
	return &SuperAdminSeatDelete{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// DeleteOne returns a builder for deleting the given entity.
func (c *SuperAdminSeatClient) DeleteOne(_m *SuperAdminSeat) *SuperAdminSeatDeleteOne {
	return c.DeleteOneID(_m.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *SuperAdminSeatClient) DeleteOneID(id int16) *SuperAdminSeatDeleteOne {
	builder := c.Delete().Where(superadminseat.ID(id))
	builder.mutation.id = &id
	builder.mutation.op = OpDeleteOne
	return &SuperAdminSeatDeleteOne{builder}
}

// Query returns a query builder for SuperAdminSeat.
func (c *SuperAdminSeatClient) Query() *SuperAdminSeatQuery {
	return &SuperAdminSeatQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeSuperAdminSeat},
		inters: c.Interceptors(),
	}
}

// Get returns a SuperAdminSeat entity by its id.
func (c *SuperAdminSeatClient) Get(ctx context.Context, id int16) (*SuperAdminSeat, error) {
	return c.Query().Where(superadminseat.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *SuperAdminSeatClient) GetX(ctx context.Context, id int16) *SuperAdminSeat {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// QueryUser queries the user edge of a SuperAdminSeat.
func (c *SuperAdminSeatClient) QueryUser(_m *SuperAdminSeat) *UserQuery {
	query := (&UserClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(superadminseat.Table, superadminseat.FieldID, id),
			sqlgraph.To(user.Table, user.FieldID),
			sqlgraph.Edge(sqlgraph.M2O, false, superadminseat.UserTable, superadminseat.UserColumn),
		)
		fromV = sqlgraph.Neighbors(_m.driver.Dialect(), step)
		return fromV, nil
	}
...
// Hooks returns the client hooks.
func (c *SuperAdminSeatClient) Hooks() []Hook {
	return c.hooks.SuperAdminSeat
}

// Interceptors returns the client interceptors.
func (c *SuperAdminSeatClient) Interceptors() []Interceptor {
	return c.inters.SuperAdminSeat
}

func (c *SuperAdminSeatClient) mutate(ctx context.Context, m *SuperAdminSeatMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&SuperAdminSeatCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&SuperAdminSeatUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&SuperAdminSeatUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&SuperAdminSeatDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown SuperAdminSeat mutation op: %q", m.Op())
	}
}
...
// WorkSessionClient is a client for the WorkSession schema.
type WorkSessionClient struct {
	config
}

// NewWorkSessionClient returns a client for the WorkSession from the given config.
func NewWorkSessionClient(c config) *WorkSessionClient {
	return &WorkSessionClient{config: c}
}

// Use adds a list of mutation hooks to the hooks stack.
// A call to `Use(f, g, h)` equals to `worksession.Hooks(f(g(h())))`.
func (c *WorkSessionClient) Use(hooks ...Hook) {
	c.hooks.WorkSession = append(c.hooks.WorkSession, hooks...)
}

// Intercept adds a list of query interceptors to the interceptors stack.
// A call to `Intercept(f, g, h)` equals to `worksession.Intercept(f(g(h())))`.
func (c *WorkSessionClient) Intercept(interceptors ...Interceptor) {
	c.inters.WorkSession = append(c.inters.WorkSession, interceptors...)
}

// Create returns a builder for creating a WorkSession entity.
func (c *WorkSessionClient) Create() *WorkSessionCreate {
	mutation := newWorkSessionMutation(c.config, OpCreate)
	return &WorkSessionCreate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// CreateBulk returns a builder for creating a bulk of WorkSession entities.
func (c *WorkSessionClient) CreateBulk(builders ...*WorkSessionCreate) *WorkSessionCreateBulk {
	return &WorkSessionCreateBulk{config: c.config, builders: builders}
}

// MapCreateBulk creates a bulk creation builder from the given slice. For each item in the slice, the function creates
// a builder and applies setFunc on it.
func (c *WorkSessionClient) MapCreateBulk(slice any, setFunc func(*WorkSessionCreate, int)) *WorkSessionCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &WorkSessionCreateBulk{err: fmt.Errorf("calling to WorkSessionClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*WorkSessionCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
		setFunc(builders[i], i)
	}
...
// Update returns an update builder for WorkSession.
func (c *WorkSessionClient) Update() *WorkSessionUpdate {
	mutation := newWorkSessionMutation(c.config, OpUpdate)
	return &WorkSessionUpdate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOne returns an update builder for the given entity.
func (c *WorkSessionClient) UpdateOne(_m *WorkSession) *WorkSessionUpdateOne {
	mutation := newWorkSessionMutation(c.config, OpUpdateOne, withWorkSession(_m))
	return &WorkSessionUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *WorkSessionClient) UpdateOneID(id uuid.UUID) *WorkSessionUpdateOne {
	mutation := newWorkSessionMutation(c.config, OpUpdateOne, withWorkSessionID(id))
	return &WorkSessionUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// Delete returns a delete builder for WorkSession.
func (c *WorkSessionClient) Delete() *WorkSessionDelete {
	mutation := newWorkSessionMutation(c.config, OpDelete)
	return &WorkSessionDelete{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// DeleteOne returns a builder for deleting the given entity.
func (c *WorkSessionClient) DeleteOne(_m *WorkSession) *WorkSessionDeleteOne {
	return c.DeleteOneID(_m.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *WorkSessionClient) DeleteOneID(id uuid.UUID) *WorkSessionDeleteOne {
	builder := c.Delete().Where(worksession.ID(id))
	builder.mutation.id = &id
	builder.mutation.op = OpDeleteOne
	return &WorkSessionDeleteOne{builder}
}

// Query returns a query builder for WorkSession.
func (c *WorkSessionClient) Query() *WorkSessionQuery {
	return &WorkSessionQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeWorkSession},
		inters: c.Interceptors(),
	}
}

// Get returns a WorkSession entity by its id.
func (c *WorkSessionClient) Get(ctx context.Context, id uuid.UUID) (*WorkSession, error) {
	return c.Query().Where(worksession.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *WorkSessionClient) GetX(ctx context.Context, id uuid.UUID) *WorkSession {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// Hooks returns the client hooks.
func (c *WorkSessionClient) Hooks() []Hook {
	return c.hooks.WorkSession
}

// Interceptors returns the client interceptors.
func (c *WorkSessionClient) Interceptors() []Interceptor {
	return c.inters.WorkSession
}

func (c *WorkSessionClient) mutate(ctx context.Context, m *WorkSessionMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&WorkSessionCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&WorkSessionUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&WorkSessionUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&WorkSessionDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown WorkSession mutation op: %q", m.Op())
	}
}
...
type (
	hooks struct {
		APIKey, Account, AccountGroup, Announcement, AnnouncementRead, AuditContentPart,
		AuditInteraction, AuthIdentity, AuthIdentityChannel, AutoCandidatePool,
		BatchImageEvent, BatchImageItem, BatchImageJob, ChannelMonitor,
		ChannelMonitorDailyRollup, ChannelMonitorHistory,
		ChannelMonitorRequestTemplate, ErrorPassthroughRule, GatewayInferenceRun,
		GovernanceEvent, Group, IdempotencyRecord, IdentityAdoptionDecision,
		ModelCatalogEntry, PaymentAuditLog, PaymentOrder, PaymentProviderInstance,
		PendingAuthSession, PromoCode, PromoCodeUsage, Proxy, RedeemCode,
		RouteDecision, SecuritySecret, Setting, SubscriptionPlan, SuperAdminSeat,
		TLSFingerprintProfile, UsageCleanupTask, UsageLog, User, UserAllowedGroup,
		UserAttributeDefinition, UserAttributeValue, UserPlatformQuota,
		UserSubscription, WorkSession []ent.Hook
	}
	inters struct {
		APIKey, Account, AccountGroup, Announcement, AnnouncementRead, AuditContentPart,
		AuditInteraction, AuthIdentity, AuthIdentityChannel, AutoCandidatePool,
		...
		TLSFingerprintProfile, UsageCleanupTask, UsageLog, User, UserAllowedGroup,
		UserAttributeDefinition, UserAttributeValue, UserPlatformQuota,
		UserSubscription, WorkSession []ent.Interceptor
	}
)
