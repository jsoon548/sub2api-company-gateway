...
func (tx *Tx) init() {
	...
	tx.AuditContentPart = NewAuditContentPartClient(tx.config)
	tx.AuditInteraction = NewAuditInteractionClient(tx.config)
	tx.AuthIdentity = NewAuthIdentityClient(tx.config)
	tx.AuthIdentityChannel = NewAuthIdentityChannelClient(tx.config)
	tx.AutoCandidatePool = NewAutoCandidatePoolClient(tx.config)
	...
	tx.GatewayInferenceRun = NewGatewayInferenceRunClient(tx.config)
	tx.GovernanceEvent = NewGovernanceEventClient(tx.config)
	...
	tx.RouteDecision = NewRouteDecisionClient(tx.config)
	tx.SecuritySecret = NewSecuritySecretClient(tx.config)
	tx.Setting = NewSettingClient(tx.config)
	tx.SubscriptionPlan = NewSubscriptionPlanClient(tx.config)
	tx.SuperAdminSeat = NewSuperAdminSeatClient(tx.config)
