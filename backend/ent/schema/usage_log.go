...
func (UsageLog) Fields() []ent.Field {
	return []ent.Field{
		...
		field.String("request_id").
			MaxLen(64).
			NotEmpty(),
		// GatewayRequestID is a nullable correlation key. audit foundation only adds the
		// storage field; usage correlation will connect usage writes to audit records.
		field.UUID("gateway_request_id", uuid.UUID{}).
			Optional().
			Nillable(),
		field.String("model").
			MaxLen(100).
...
func (UsageLog) Indexes() []ent.Index {
	return []ent.Index{
		...
		index.Fields("gateway_request_id").
			StorageKey("usage_logs_gateway_request_id_idx").
			Annotations(entsql.IndexWhere("gateway_request_id IS NOT NULL")),
		// 复合索引用于时间范围查询
