		...
		field.String("role").
			...
			Validate(func(value string) error {
				...
				case domain.RoleSuperAdmin, domain.RoleAdmin, domain.RoleUser:
					return nil
				default:
					return fmt.Errorf("must be one of super_admin, admin, user")
				}
			}).
			Default(domain.RoleUser),
		field.Int64("session_version").
			Default(0).
			Comment("Persisted security epoch for immediate session and credential revocation"),
		field.Float("balance").
