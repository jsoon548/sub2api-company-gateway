...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	governanceRepository := repository.NewGovernanceRepository(db)
	proxyExitInfoProber := repository.NewProxyExitInfoProber(configConfig)
	proxyLatencyCache := repository.NewProxyLatencyCache(redisClient)
	adminService := service.NewAdminService(userRepository, governanceRepository, groupRepository, accountRepository, proxyRepository, apiKeyRepository, redeemCodeRepository, userGroupRateRepository, userRPMCache, billingCacheService, proxyExitInfoProber, proxyLatencyCache, apiKeyAuthCacheInvalidator, client, settingService, subscriptionService, userSubscriptionRepository, privacyClientFactory, openAIGatewayService)
	...
	superAdminTransferService := service.NewSuperAdminTransferService(governanceRepository, authService, apiKeyAuthCacheInvalidator)
	governanceHandler := admin.NewGovernanceHandler(superAdminTransferService)
	auditManagementRepository := repository.NewAuditManagementRepository(db)
	auditFoundationRepository := repository.NewAuditFoundationRepository(db)
	auditFoundationService := service.ProvideAuditFoundationService(auditFoundationRepository, configConfig)
	auditManagementService := service.ProvideAuditManagementService(auditManagementRepository, auditFoundationService)
	auditManagementHandler := admin.NewAuditManagementHandler(auditManagementService)
	workSessionRepository := repository.NewWorkSessionRepository(db)
	runtime := service.ProvideInternalInferenceRuntime(configConfig)
	workSessionService := service.ProvideWorkSessionService(workSessionRepository, configConfig, runtime)
	workSessionHandler := admin.NewWorkSessionHandler(workSessionService)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, paymentHandler, affiliateHandler, complianceHandler, governanceHandler, auditManagementHandler, workSessionHandler)
	...
	explicitModelResolver := service.ProvideExplicitModelResolver(groupRepository, accountRepository, channelService)
	engine := server.ProvideRouter(configConfig, handlers, jwtAuthMiddleware, adminAuthMiddleware, apiKeyAuthMiddleware, apiKeyService, subscriptionService, opsService, settingService, explicitModelResolver, auditFoundationService, workSessionService, redisClient)
