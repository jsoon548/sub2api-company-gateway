...
func TestLoadAuditFoundationConfigUsesReferenceOnly(t *testing.T) {
	resetViperWithJWTSecret(t)
	t.Setenv("AUDIT_MODE", "required")
	t.Setenv("AUDIT_CONTENT_KEY_REF", "env:AUDIT_AUDIT_SYNTHETIC_CONTENT_KEY")
	t.Setenv("AUDIT_CONTENT_KEY_VERSION", "auditFoundation-v1")
	t.Setenv("AUDIT_RECONCILE_STALE_AFTER_SECONDS", "120")
	t.Setenv("AUDIT_RECONCILE_INTERVAL_SECONDS", "30")

	cfg, err := Load()
	require.NoError(t, err)
	require.Equal(t, "required", cfg.Audit.Mode)
	require.Equal(t, "env:AUDIT_AUDIT_SYNTHETIC_CONTENT_KEY", cfg.Audit.ContentKeyRef)
	require.Equal(t, "auditFoundation-v1", cfg.Audit.ContentKeyVersion)
	require.Equal(t, 120, cfg.Audit.ReconcileStaleAfterSeconds)
	require.Equal(t, 30, cfg.Audit.ReconcileIntervalSeconds)
}

func TestLoadAuditFoundationDefaultsToDisabled(t *testing.T) {
	resetViperWithJWTSecret(t)
	cfg, err := Load()
	require.NoError(t, err)
	require.Equal(t, "disabled", cfg.Audit.Mode)
	require.Empty(t, cfg.Audit.ContentKeyRef)
	require.Empty(t, cfg.Audit.ContentKeyVersion)
}

func TestLoadWorkSessionConfigUsesIndependentReferenceAndVersion(t *testing.T) {
	resetViperWithJWTSecret(t)
	t.Setenv("WORK_SESSION_MODE", "required")
	t.Setenv("WORK_SESSION_TENANT_ID", "tenant-synthetic")
	t.Setenv("WORK_SESSION_HMAC_KEY_REF", "env:WORK_SESSION_WORK_SESSION_HMAC_SYNTHETIC_KEY")
	t.Setenv("WORK_SESSION_HMAC_KEY_VERSION", "workSession-v1")

	cfg, err := Load()
	require.NoError(t, err)
	require.Equal(t, "required", cfg.WorkSession.Mode)
	require.Equal(t, "tenant-synthetic", cfg.WorkSession.TenantID)
	require.Equal(t, "env:WORK_SESSION_WORK_SESSION_HMAC_SYNTHETIC_KEY", cfg.WorkSession.HMACKeyRef)
	require.Equal(t, "workSession-v1", cfg.WorkSession.HMACKeyVersion)
}

func TestLoadInternalInferenceAutoComplexityConfig(t *testing.T) {
	resetViperWithJWTSecret(t)
	t.Setenv("INTERNAL_INFERENCE_BACKEND_NAME", " synthetic-backend ")
	t.Setenv("INTERNAL_INFERENCE_BACKEND_TYPE", " OPENAI_COMPATIBLE_CHAT_COMPLETIONS ")
	t.Setenv("INTERNAL_INFERENCE_BACKEND_PROVIDER", " synthetic-provider ")
	t.Setenv("INTERNAL_INFERENCE_BACKEND_BASE_URL", " http://127.0.0.1:61234/v1 ")
	t.Setenv("INTERNAL_INFERENCE_BACKEND_ALLOWED_HOST", " 127.0.0.1 ")
	t.Setenv("INTERNAL_INFERENCE_BACKEND_ALLOW_PRIVATE_NETWORK_HTTP", "true")
	t.Setenv("INTERNAL_INFERENCE_BACKEND_CREDENTIAL_REF", " env:INTERNAL_INFERENCE_INTERNAL_INFERENCE_SYNTHETIC_CREDENTIAL ")
	t.Setenv("INTERNAL_INFERENCE_PROFILES_AUTO_COMPLEXITY_ENABLED", "true")
	t.Setenv("INTERNAL_INFERENCE_PROFILES_AUTO_COMPLEXITY_BACKEND", " synthetic-backend ")
	t.Setenv("INTERNAL_INFERENCE_PROFILES_AUTO_COMPLEXITY_MODEL", " qwen3-vl-32b-instruct ")
	t.Setenv("INTERNAL_INFERENCE_PROFILES_AUTO_COMPLEXITY_DISABLE_THINKING", "true")
	t.Setenv("INTERNAL_INFERENCE_PROFILES_AUTO_COMPLEXITY_TIMEOUT_MS", "2000")
	t.Setenv("INTERNAL_INFERENCE_PROFILES_AUTO_COMPLEXITY_MAX_INPUT_BYTES", "8192")
	t.Setenv("INTERNAL_INFERENCE_PROFILES_AUTO_COMPLEXITY_MAX_OUTPUT_UNITS", "64")
	t.Setenv("INTERNAL_INFERENCE_PROFILES_AUTO_COMPLEXITY_MAX_RESPONSE_BYTES", "32768")
	t.Setenv("INTERNAL_INFERENCE_PROFILES_AUTO_COMPLEXITY_PROMPT_VERSION", " auto-complexity-v2 ")
	t.Setenv("INTERNAL_INFERENCE_PROFILES_AUTO_COMPLEXITY_SCHEMA_VERSION", " auto-complexity-v2 ")

	cfg, err := Load()
	require.NoError(t, err)
	require.Equal(t, "synthetic-backend", cfg.InternalInference.Backend.Name)
	require.Equal(t, "openai_compatible_chat_completions", cfg.InternalInference.Backend.Type)
	require.Equal(t, "synthetic-provider", cfg.InternalInference.Backend.Provider)
	require.Equal(t, "http://127.0.0.1:61234/v1", cfg.InternalInference.Backend.BaseURL)
	require.Equal(t, "127.0.0.1", cfg.InternalInference.Backend.AllowedHost)
	require.True(t, cfg.InternalInference.Backend.AllowPrivateNetworkHTTP)
	require.Equal(t, "env:INTERNAL_INFERENCE_INTERNAL_INFERENCE_SYNTHETIC_CREDENTIAL", cfg.InternalInference.Backend.CredentialRef)
	profile := cfg.InternalInference.Profiles.AutoComplexity
	require.True(t, profile.Enabled)
	require.Equal(t, "synthetic-backend", profile.Backend)
	require.Equal(t, "qwen3-vl-32b-instruct", profile.Model)
	require.True(t, profile.DisableThinking)
	require.Equal(t, 2000, profile.TimeoutMS)
	require.Equal(t, 8192, profile.MaxInputBytes)
	require.Equal(t, 64, profile.MaxOutputUnits)
	require.EqualValues(t, 32768, profile.MaxResponseBytes)
	require.Equal(t, "auto-complexity-v2", profile.PromptVersion)
	require.Equal(t, "auto-complexity-v2", profile.SchemaVersion)
}

func TestLoadWorkSessionDefaultsToDisabledWithoutSecretMaterial(t *testing.T) {
	resetViperWithJWTSecret(t)
	cfg, err := Load()
	require.NoError(t, err)
	require.Equal(t, "disabled", cfg.WorkSession.Mode)
	require.Empty(t, cfg.WorkSession.TenantID)
	require.Empty(t, cfg.WorkSession.HMACKeyRef)
	require.Empty(t, cfg.WorkSession.HMACKeyVersion)
}
