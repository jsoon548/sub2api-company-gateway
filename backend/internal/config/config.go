...
type Config struct {
	...
	WorkSession             WorkSessionConfig             `mapstructure:"work_session"`
	InternalInference       InternalInferenceConfig       `mapstructure:"internal_inference"`
...
// AuditConfig contains only deployment references and reconciliation timing.
// The content key itself is resolved from the referenced environment variable
// by the audit foundation service and is never stored in Config.
type AuditConfig struct {
	Mode                       string `mapstructure:"mode"`
	ContentKeyRef              string `mapstructure:"content_key_ref"`
	ContentKeyVersion          string `mapstructure:"content_key_version"`
	ReconcileStaleAfterSeconds int    `mapstructure:"reconcile_stale_after_seconds"`
	ReconcileIntervalSeconds   int    `mapstructure:"reconcile_interval_seconds"`
}

// WorkSessionConfig contains deployment-owned identity material for reliable
// Work Sessions. The HMAC key is resolved from HMACKeyRef by the service and
// is never stored in Config or generated by the application.
type WorkSessionConfig struct {
	Mode           string `mapstructure:"mode"`
	TenantID       string `mapstructure:"tenant_id"`
	HMACKeyRef     string `mapstructure:"hmac_key_ref"`
	HMACKeyVersion string `mapstructure:"hmac_key_version"`
}

// InternalInferenceConfig is deployment-owned. Runtime requests cannot change
// the backend URL, model, credential reference, or versioned profile policy.
type InternalInferenceConfig struct {
	Backend  InternalInferenceBackendConfig  `mapstructure:"backend"`
	Profiles InternalInferenceProfilesConfig `mapstructure:"profiles"`
}

type InternalInferenceBackendConfig struct {
	Name                    string `mapstructure:"name"`
	Type                    string `mapstructure:"type"`
	Provider                string `mapstructure:"provider"`
	BaseURL                 string `mapstructure:"base_url"`
	AllowedHost             string `mapstructure:"allowed_host"`
	AllowPrivateNetworkHTTP bool   `mapstructure:"allow_private_network_http"`
	CredentialRef           string `mapstructure:"credential_ref"`
}

type InternalInferenceProfilesConfig struct {
	AutoComplexity InternalInferenceProfileConfig `mapstructure:"auto_complexity"`
}

type InternalInferenceProfileConfig struct {
	Enabled          bool   `mapstructure:"enabled"`
	Backend          string `mapstructure:"backend"`
	Model            string `mapstructure:"model"`
	DisableThinking  bool   `mapstructure:"disable_thinking"`
	TimeoutMS        int    `mapstructure:"timeout_ms"`
	MaxInputBytes    int    `mapstructure:"max_input_bytes"`
	MaxOutputUnits   int    `mapstructure:"max_output_units"`
...
func load(allowMissingJWTSecret bool) (*Config, error) {
	...
	cfg.Audit.Mode = strings.ToLower(strings.TrimSpace(cfg.Audit.Mode))
	cfg.Audit.ContentKeyRef = strings.TrimSpace(cfg.Audit.ContentKeyRef)
	cfg.Audit.ContentKeyVersion = strings.TrimSpace(cfg.Audit.ContentKeyVersion)
	cfg.WorkSession.Mode = strings.ToLower(strings.TrimSpace(cfg.WorkSession.Mode))
	cfg.WorkSession.TenantID = strings.TrimSpace(cfg.WorkSession.TenantID)
	cfg.WorkSession.HMACKeyRef = strings.TrimSpace(cfg.WorkSession.HMACKeyRef)
	cfg.WorkSession.HMACKeyVersion = strings.TrimSpace(cfg.WorkSession.HMACKeyVersion)
	cfg.InternalInference.Backend.Name = strings.TrimSpace(cfg.InternalInference.Backend.Name)
	cfg.InternalInference.Backend.Type = strings.ToLower(strings.TrimSpace(cfg.InternalInference.Backend.Type))
	cfg.InternalInference.Backend.Provider = strings.TrimSpace(cfg.InternalInference.Backend.Provider)
	cfg.InternalInference.Backend.BaseURL = strings.TrimSpace(cfg.InternalInference.Backend.BaseURL)
	cfg.InternalInference.Backend.AllowedHost = strings.ToLower(strings.TrimSpace(cfg.InternalInference.Backend.AllowedHost))
	cfg.InternalInference.Backend.CredentialRef = strings.TrimSpace(cfg.InternalInference.Backend.CredentialRef)
	cfg.InternalInference.Profiles.AutoComplexity.Backend = strings.TrimSpace(cfg.InternalInference.Profiles.AutoComplexity.Backend)
	cfg.InternalInference.Profiles.AutoComplexity.Model = strings.TrimSpace(cfg.InternalInference.Profiles.AutoComplexity.Model)
	cfg.InternalInference.Profiles.AutoComplexity.PromptVersion = strings.TrimSpace(cfg.InternalInference.Profiles.AutoComplexity.PromptVersion)
	cfg.InternalInference.Profiles.AutoComplexity.SchemaVersion = strings.TrimSpace(cfg.InternalInference.Profiles.AutoComplexity.SchemaVersion)
...
func setDefaults() {
	...
	// PostgreSQL-only audit foundation. disabled is fail-closed for every
	// content-bearing Gateway entry while keeping ordinary Web available.
	viper.SetDefault("audit.mode", "disabled")
	viper.SetDefault("audit.content_key_ref", "")
	viper.SetDefault("audit.content_key_version", "")
	viper.SetDefault("audit.reconcile_stale_after_seconds", 900)
	viper.SetDefault("audit.reconcile_interval_seconds", 60)

	// Reliable Work Sessions are deployment-enabled independently of audit.
	// Explicit model requests and the ordinary Web application remain available
	// when this capability is disabled or its key is unavailable.
	viper.SetDefault("work_session.mode", "disabled")
	viper.SetDefault("work_session.tenant_id", "")
	viper.SetDefault("work_session.hmac_key_ref", "")
	viper.SetDefault("work_session.hmac_key_version", "")

	// Gateway Internal Inference is optional and never gates normal startup.
	// The internal-inference profile is explicitly disabled until deployment
	// supplies an approved backend and an environment-owned credential reference.
	viper.SetDefault("internal_inference.backend.name", "openai-compatible-primary")
	viper.SetDefault("internal_inference.backend.type", "openai_compatible_chat_completions")
	viper.SetDefault("internal_inference.backend.provider", "openai-compatible")
	viper.SetDefault("internal_inference.backend.base_url", "")
	viper.SetDefault("internal_inference.backend.allowed_host", "")
	viper.SetDefault("internal_inference.backend.allow_private_network_http", false)
	viper.SetDefault("internal_inference.backend.credential_ref", "")
	viper.SetDefault("internal_inference.profiles.auto_complexity.enabled", false)
	viper.SetDefault("internal_inference.profiles.auto_complexity.backend", "openai-compatible-primary")
	viper.SetDefault("internal_inference.profiles.auto_complexity.model", "")
	viper.SetDefault("internal_inference.profiles.auto_complexity.disable_thinking", false)
	viper.SetDefault("internal_inference.profiles.auto_complexity.timeout_ms", 2000)
	viper.SetDefault("internal_inference.profiles.auto_complexity.max_input_bytes", 16*1024)
	viper.SetDefault("internal_inference.profiles.auto_complexity.max_output_units", 64)
	viper.SetDefault("internal_inference.profiles.auto_complexity.max_response_bytes", 64*1024)
	viper.SetDefault("internal_inference.profiles.auto_complexity.prompt_version", "auto-complexity-v2")
	viper.SetDefault("internal_inference.profiles.auto_complexity.schema_version", "auto-complexity-v2")
