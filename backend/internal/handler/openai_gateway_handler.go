...
func usageRecordContext(parent context.Context, base context.Context) context.Context {
	...
	if gatewayRequestID, ok := parent.Value(ctxkey.GatewayRequestID).(uuid.UUID); ok && gatewayRequestID != uuid.Nil {
		base = context.WithValue(base, ctxkey.GatewayRequestID, gatewayRequestID)
	}
	if workSessionID, ok := parent.Value(ctxkey.WorkSessionID).(uuid.UUID); ok && workSessionID != uuid.Nil {
		base = context.WithValue(base, ctxkey.WorkSessionID, workSessionID)
	}
	return base
}
...
func (h *OpenAIGatewayHandler) Responses(c *gin.Context) {
	...
	channelMapping = service.ApplyExplicitModelResolution(c.Request.Context(), reqModel, channelMapping)
				...
				if errors.As(err, &failoverErr) {
					...
					if failoverErr.RetryableOnSameAccount {
						...
						if sameAccountRetryCount[account.ID] < retryLimit {
							if !service.ClaimAutoTechnicalRetry(c.Request.Context(), false, "same_account_retry") {
					...
					if !service.ClaimAutoTechnicalRetry(c.Request.Context(), false, "account_switch") {
						h.handleFailoverExhausted(c, failoverErr, streamStarted)
						return
					}
					service.ApplyAutoRouteResponseHeaders(c.Writer.Header(), c.Request.Context())
...
func (h *OpenAIGatewayHandler) normalizeOpenAIResponsesCompactRequest(c *gin.Context, reqLog *zap.Logger, body []byte) ([]byte, bool) {
	...
	// Compact always uses unary JSON upstream after whitelist normalization.
	// Preserve the original client transport for both the body-signal and the
	// exact client profile path profiles so stream:true receives the SSE bridge rather
	// than an unexpected JSON response.
	if clientStream {
...
func (h *OpenAIGatewayHandler) Messages(c *gin.Context) {
	...
	channelMappingMsg = service.ApplyExplicitModelResolution(c.Request.Context(), reqModel, channelMappingMsg)
