...
func TestUsageRecordContextPreservesGatewayCorrelationSeparately(t *testing.T) {
	gatewayID := uuid.New()
	parent := context.WithValue(context.Background(), ctxkey.GatewayRequestID, gatewayID)
	parent = context.WithValue(parent, ctxkey.RequestID, "legacy-request")

	got := usageRecordContext(parent, context.Background())
	require.Equal(t, gatewayID, got.Value(ctxkey.GatewayRequestID))
	require.Equal(t, "legacy-request", got.Value(ctxkey.RequestID))
}
