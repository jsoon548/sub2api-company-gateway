...
func (h *GatewayHandler) CountTokens(c *gin.Context) {
	...
	channelMapping = service.ApplyExplicitModelResolution(c.Request.Context(), requestedModel, channelMapping)
	...
	// 选择支持该模型的账号
	account, err := h.gatewayService.SelectAccountForModel(c.Request.Context(), apiKey.GroupID, sessionHash, requestedModel)
	if err != nil {
	...
	if channelMapping.Mapped {
		...
		if err := parsedReq.ReplaceBody(h.gatewayService.ReplaceModelInBody(parsedReq.Body.Bytes(), channelMapping.MappedModel)); err != nil {
			h.errorResponse(c, http.StatusInternalServerError, "api_error", "Failed to apply approved model mapping")
			return
		}
	}
