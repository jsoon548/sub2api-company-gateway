...
func TestRedactCredentials_APIKeyReferenceReportsAvailabilityWithoutSecret(t *testing.T) {
	out, status := RedactCredentials(map[string]any{
		"api_key_ref": "env:SUB2API_PROVIDER_INTERNAL_INFERENCE_API_KEY",
		"base_url":    "https://provider.example/v1",
	})

	require.Equal(t, "env:SUB2API_PROVIDER_INTERNAL_INFERENCE_API_KEY", out["api_key_ref"])
	require.Equal(t, "https://provider.example/v1", out["base_url"])
	require.True(t, status["has_api_key"])
}
