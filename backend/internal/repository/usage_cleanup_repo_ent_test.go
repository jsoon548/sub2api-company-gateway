...
func TestUsageCleanupRepositoryEntCreateAndList(t *testing.T) {
	...
	secondTask := &service.UsageCleanupTask{
...
func TestUsageCleanupRepositoryEntMarkResults(t *testing.T) {
	...
	require.NoError(t, repo.MarkTaskFailed(context.Background(), secondTask.ID, 4, "boom"))
	loaded2, err := client.UsageCleanupTask.Get(context.Background(), secondTask.ID)
	require.NoError(t, err)
