...
func (s *APIKeyRepoSuite) TestUpdate_DoesNotOverwriteConcurrentLifecycleRevocation() {
	user := s.mustCreateUser("lifecycle-revoked@test.com")
	key := &service.APIKey{
		UserID: user.ID,
	...
	stale.Status = service.StatusAPIKeyActive

	err := s.repo.Update(s.ctx, &stale)

	s.Require().ErrorIs(err, service.ErrAPIKeyLifecycleRevoked)
	got, getErr := s.repo.GetByID(s.ctx, key.ID)
	s.Require().NoError(getErr)
	s.Require().Equal(service.StatusAPIKeyDisabled, got.Status)
	s.Require().Equal("Original", got.Name, "stale update must not overwrite any fields")
}
