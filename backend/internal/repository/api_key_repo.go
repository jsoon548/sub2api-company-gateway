...
func (r *apiKeyRepository) Update(ctx context.Context, key *service.APIKey) error {
	...
	builder := client.APIKey.Update().
		...
		SetUpdatedAt(now)
	// A lifecycle-disabled key is terminal. Keep this predicate at the write
	// boundary so an API-key edit racing with account deactivation cannot write
	// an older active/inactive snapshot over the lifecycle revocation.
	if key.Status != service.StatusAPIKeyDisabled {
		builder.Where(apikey.StatusNEQ(service.StatusAPIKeyDisabled))
	}
	if key.GroupID != nil {
	...
	if affected == 0 {
		// Distinguish terminal lifecycle revocation from not-found/soft-delete.
		// This query only runs on the exceptional zero-row path.
		current, queryErr := client.APIKey.Query().
			Where(apikey.IDEQ(key.ID), apikey.DeletedAtIsNil()).
			Only(ctx)
		if queryErr == nil && current.Status == service.StatusAPIKeyDisabled && key.Status != service.StatusAPIKeyDisabled {
			return service.ErrAPIKeyLifecycleRevoked
		}
