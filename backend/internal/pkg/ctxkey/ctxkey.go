...
const (
	...
	// GatewayRequestID is the Gateway-owned correlation UUID used to link
	// authoritative audit metadata with the independently persisted usage row.
	// It is deliberately separate from RequestID and ClientRequestID.
	GatewayRequestID Key = "ctx_gateway_request_id"

	// WorkSessionID is the Gateway-owned reliable or request-scoped Work
	// Session correlation UUID. It is distinct from scheduling affinity keys.
	WorkSessionID Key = "ctx_work_session_id"
