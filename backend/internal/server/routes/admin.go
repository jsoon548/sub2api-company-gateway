...
func RegisterAdminRoutes(
	...
	admin.Use(middleware.AdminComplianceGuard(settingService))
	{
		...
		// Core Gateway 审计 metadata、受控原文查看
		registerAuditManagementRoutes(admin, h)

		// Core Gateway usage 关联与最小管理视图
		registerGatewayUsageRoutes(admin, h)

		// Core Gateway Work Session identity, model catalog, and Auto pilot configuration.
		registerWorkSessionRoutes(admin, h)
...
func registerWorkSessionRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	workSessions := admin.Group("/work-session-auto")
	workSessions.Use(middleware.RequireCapability(service.CapabilityAdminAccess, true))
	{
		workSessions.GET("", h.Admin.WorkSession.GetManagementState)
		workSessions.PUT(
			"/config",
			middleware.RequireCapability(service.CapabilityManageModelCatalog, true),
			middleware.RequireCapability(service.CapabilityManageAutoConfig, true),
			h.Admin.WorkSession.ReplaceManagementConfig,
		)
		workSessions.PUT(
			"/models/:logical_model/emergency-disable",
			middleware.RequireCapability(service.CapabilityManageModelCatalog, true),
			h.Admin.WorkSession.SetEmergencyModelDisable,
		)
	}
}

func registerGatewayUsageRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	usage := admin.Group("/gateway-usage")
	usage.Use(middleware.RequireCapability(service.CapabilityAdminAccess, true))
	{
		usage.GET("", h.Admin.AuditManagement.ListGatewayUsage)
		usage.GET("/summary", h.Admin.AuditManagement.SummarizeGatewayUsage)
		usage.GET("/:gateway_request_id", h.Admin.AuditManagement.GetGatewayUsage)
	}
}

func registerAuditManagementRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	audit := admin.Group("/audit")
	audit.Use(middleware.RequireCapability(service.CapabilityAdminAccess, true))
	{
		audit.GET("/interactions", h.Admin.AuditManagement.ListMetadata)
		audit.POST(
			"/interactions/:id/disclose",
			middleware.RequireCapability(service.CapabilityRawContentDisclosure, true),
			h.Admin.AuditManagement.Disclose,
		)
	}
}
...
func registerUserManagementRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	users := admin.Group("/users")
	{
		...
		users.POST("/:id/deactivate", h.Admin.Governance.DeactivateUser)
		users.POST("/:id/reactivate", h.Admin.Governance.ReactivateUser)
...
func registerGovernanceRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	...
	governance.Use(middleware.RequireCapability(service.CapabilityTransferSuperAdmin, true))
	{
		governance.GET("/super-admin/seat", h.Admin.Governance.GetSuperAdminSeat)
		governance.POST("/super-admin/transfer", h.Admin.Governance.TransferSuperAdmin)
	}
}
