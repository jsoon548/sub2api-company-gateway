...
func adminAuth(
	...
	return func(c *gin.Context) {
		...
		if apiKey != "" {
			...
			if !sharedAdminAPIKeyRouteAllowed(c.Request.Method, c.Request.URL.Path) {
				AbortWithError(c, 403, "SHARED_ADMIN_KEY_FORBIDDEN", "Shared admin API key is not authorized for this route")
				return
			}
			c.Next()
			return
...
func validateAdminAPIKey(
	...
	// A shared key proves possession of one deployment secret, not a named
	// administrator identity. Never map it to the first administrator.
	c.Set(string(ContextKeyUser), AuthSubject{})
	c.Set("auth_method", "admin_api_key")
	return true
}

func sharedAdminAPIKeyRouteAllowed(method, path string) bool {
	if method != "GET" {
		return false
	}
	switch path {
