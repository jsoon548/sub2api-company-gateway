...
func TestAdminAuthJWTValidatesTokenVersion(t *testing.T) {
	...
	router.GET("/t", func(c *gin.Context) {
		...
		c.JSON(http.StatusOK, gin.H{"ok": true, "session_version": subject.SessionVersion})
	})
	...
	t.Run("token_version_match_allows", func(t *testing.T) {
		...
		require.Contains(t, w.Body.String(), `"session_version":0`)
	})

	t.Run("expired_named_session_rejected", func(t *testing.T) {
		claims := service.JWTClaims{
			UserID: admin.ID, Email: admin.Email, Role: service.RoleSuperAdmin,
			TokenVersion: admin.TokenVersion,
		...
		require.Equal(t, http.StatusUnauthorized, w.Code)
		require.Contains(t, w.Body.String(), "TOKEN_EXPIRED")
	})
