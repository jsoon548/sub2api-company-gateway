...
// ProvideExplicitModelResolver wires the explicit-model contract read-only adapter over the
// existing Group/channel/account configuration. It creates no credential or
// catalog persistence dependency.
func ProvideExplicitModelResolver(
	groupRepo GroupRepository,
	accountRepo AccountRepository,
	channelService *ChannelService,
) *ExplicitModelResolver {
	return NewExplicitModelResolver(NewExplicitModelCatalogAdapter(groupRepo, accountRepo, channelService))
}

// ProvideAuditFoundationService runs the audit foundation preflight and stale-state
// reconciliation without enabling Gateway admission or content persistence.
func ProvideAuditFoundationService(
	repo AuditFoundationRepository,
	cfg *config.Config,
) *AuditFoundationService {
	svc := NewAuditFoundationService(repo, cfg.Audit)
	svc.Start()
	return svc
}

// ProvideAuditManagementService starts the audit management retention worker. Metadata
// remains available independently of the audit content key, while disclosure
// resolves the ready codec through AuditFoundationService.
func ProvideAuditManagementService(
	repo AuditManagementRepository,
	foundation *AuditFoundationService,
) *AuditManagementService {
	svc := NewAuditManagementService(repo, foundation)
	svc.Start()
	return svc
}

// ProvideWorkSessionService performs the independent work-session implementation schema/key
// preflight. It never generates or rotates Work Session HMAC material.
func ProvideWorkSessionService(
	repo WorkSessionRepository,
	cfg *config.Config,
	internalInferenceRuntime *inference.Runtime,
) *WorkSessionService {
	svc := NewWorkSessionService(repo, cfg.WorkSession, cfg.Audit, cfg.InternalInference, internalInferenceRuntime)
	svc.Start()
	return svc
}

func ProvideInternalInferenceRuntime(cfg *config.Config) *inference.Runtime {
	return inference.NewRuntime(cfg.InternalInference, cfg.Server)
}
...
var ProviderSet = wire.NewSet(
	...
	ProvideExplicitModelResolver,
	ProvideAuditFoundationService,
	ProvideAuditManagementService,
	ProvideInternalInferenceRuntime,
	ProvideWorkSessionService,
	NewGroupService,
	NewAccountService,
	NewProxyService,
	NewRedeemService,
