...
func (s *OpenAIGatewayService) bufferChatCompletionsAsResponses(
	...
	responsesBody, marshalErr := json.Marshal(responsesResp)
	if marshalErr != nil {
		writeOpenAIResponsesFallbackError(c, http.StatusBadGateway, "upstream_error", "Failed to encode upstream response")
		return nil, fmt.Errorf("marshal responses fallback response: %w", marshalErr)
	}
	if !writeOpenAICompactSSEBridge(c, http.StatusOK, responsesBody) {
