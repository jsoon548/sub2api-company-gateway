...
func TestAPIKeyService_GetByKey_L1CacheCannotBypassOwnerLifecycle(t *testing.T) {
	currentOwner := &User{ID: 3, Status: StatusActive, Role: RoleUser, SessionVersion: 1}
	ownerRepo := &userRepoStub{user: currentOwner}
	var keyRepoCalls int32
	keyRepo := &authRepoStub{getByKeyForAuth: func(context.Context, string) (*APIKey, error) {
		atomic.AddInt32(&keyRepoCalls, 1)
		return &APIKey{
			ID: 21, UserID: currentOwner.ID, Status: StatusAPIKeyActive,
			User: &User{ID: currentOwner.ID, Status: StatusActive, Role: RoleUser, SessionVersion: 1},
		}, nil
	}}
	svc := NewAPIKeyService(keyRepo, ownerRepo, nil, nil, nil, &authCacheStub{}, &config.Config{
		APIKeyAuth: config.APIKeyAuthCacheConfig{L1Size: 1000, L1TTLSeconds: 60},
	})

	active, err := svc.GetByKey(context.Background(), "synthetic-l1-key")
	require.NoError(t, err)
	require.Equal(t, currentOwner.ID, active.User.ID)
	require.Equal(t, StatusActive, active.User.Status)
	svc.authCacheL1.Wait()

	currentOwner.Status = StatusDisabled
	inactive, err := svc.GetByKey(context.Background(), "synthetic-l1-key")
	require.NoError(t, err)
	require.Equal(t, StatusDisabled, inactive.User.Status)
	require.Equal(t, int32(1), atomic.LoadInt32(&keyRepoCalls), "second lookup must be an L1 hit")

	ownerRepo.getErr = ErrUserNotFound
	_, err = svc.GetByKey(context.Background(), "synthetic-l1-key")
	require.Error(t, err, "deleted or missing owner must fail closed even on an L1 hit")
}

func TestAPIKeyService_GetByKey_L2CacheCannotReviveOldKeyAfterReactivation(t *testing.T) {
	currentOwner := &User{ID: 8, Status: StatusActive, Role: RoleUser, SessionVersion: 1}
	ownerRepo := &userRepoStub{user: currentOwner}
	cache := &authCacheStub{}
	oldSnapshot := &APIKeyAuthCacheEntry{Snapshot: &APIKeyAuthSnapshot{
		Version: apiKeyAuthSnapshotVersion, APIKeyID: 31, UserID: currentOwner.ID, Status: StatusAPIKeyActive,
		User: APIKeyAuthUserSnapshot{ID: currentOwner.ID, Status: StatusActive, Role: RoleUser, SessionVersion: 1},
	}}
	newSnapshot := &APIKeyAuthCacheEntry{Snapshot: &APIKeyAuthSnapshot{
		Version: apiKeyAuthSnapshotVersion, APIKeyID: 32, UserID: currentOwner.ID, Status: StatusAPIKeyActive,
		User: APIKeyAuthUserSnapshot{ID: currentOwner.ID, Status: StatusActive, Role: RoleUser, SessionVersion: 2},
	}}
	keyRepo := &authRepoStub{getByKeyForAuth: func(context.Context, string) (*APIKey, error) {
		return nil, errors.New("unexpected repository lookup on L2 hit")
	}}
	svc := NewAPIKeyService(keyRepo, ownerRepo, nil, nil, nil, cache, &config.Config{
		APIKeyAuth: config.APIKeyAuthCacheConfig{L2TTLSeconds: 60},
	})
	newCacheKey := svc.authCacheKey("synthetic-new-key")
	cache.getAuthCache = func(_ context.Context, key string) (*APIKeyAuthCacheEntry, error) {
		if key == newCacheKey {
	...
	active, err := svc.GetByKey(context.Background(), "synthetic-old-key")
	require.NoError(t, err)
	require.Equal(t, StatusActive, active.User.Status)

	currentOwner.Status = StatusDisabled
	inactive, err := svc.GetByKey(context.Background(), "synthetic-old-key")
	require.NoError(t, err)
	require.Equal(t, StatusDisabled, inactive.User.Status)

	currentOwner.Status = StatusActive
	currentOwner.SessionVersion = 2
	oldKey, err := svc.GetByKey(context.Background(), "synthetic-old-key")
	require.NoError(t, err)
	require.Equal(t, StatusAPIKeyDisabled, oldKey.Status, "reactivation must not revive the pre-deactivation key")

	newKey, err := svc.GetByKey(context.Background(), "synthetic-new-key")
	require.NoError(t, err)
	require.Equal(t, StatusAPIKeyActive, newKey.Status, "a newly issued key at the current epoch is usable")
	require.Equal(t, int64(2), newKey.User.SessionVersion)
}
