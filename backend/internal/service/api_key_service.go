...
var (
	...
	ErrAPIKeyLifecycleRevoked = infraerrors.Forbidden(
		"API_KEY_LIFECYCLE_REVOKED",
		"api key was revoked when the account was disabled and cannot be reactivated",
	)
...
func (s *APIKeyService) GetByKey(ctx context.Context, key string) (*APIKey, error) {
	...
	if entry, ok := s.getAuthCacheEntry(ctx, cacheKey); ok {
		if apiKey, used, err := s.applyAuthCacheEntry(key, entry); used {
			...
			s.compileAPIKeyIPRules(apiKey)
			return s.enforceCurrentCredentialEpoch(ctx, apiKey)
		}
	}
...
// enforceCurrentCredentialEpoch keeps account deactivation authoritative even
// when an old API-key snapshot remains in L1/L2 cache. Deactivation advances
// users.session_version and disables active keys in one transaction. A later
// account reactivation does not rewind the epoch, so a pre-deactivation key can
// never become usable merely because a stale cache entry survived invalidation.
func (s *APIKeyService) enforceCurrentCredentialEpoch(ctx context.Context, apiKey *APIKey) (*APIKey, error) {
	if apiKey == nil || apiKey.User == nil || s.userRepo == nil {
		return apiKey, nil
	}
	current, err := s.userRepo.GetByID(ctx, apiKey.UserID)
	if err != nil {
		return nil, fmt.Errorf("get current api key owner: %w", err)
	}
	if current.SessionVersion != apiKey.User.SessionVersion {
		apiKey.Status = StatusAPIKeyDisabled
	}
	// Preserve the key-specific RPM override while refreshing every
	// authoritative user field, especially role and lifecycle status.
	current.UserGroupRPMOverride = apiKey.User.UserGroupRPMOverride
	apiKey.User = current
	return apiKey, nil
}
...
func (s *APIKeyService) Update(ctx context.Context, id int64, userID int64, req UpdateAPIKeyRequest) (*APIKey, error) {
	...
	// Account lifecycle revocation is terminal for the existing key. Users may
	// still toggle ordinary keys between inactive and active, but a key disabled
	// by account deactivation must be replaced with a newly issued key.
	if req.Status != nil && apiKey.Status == StatusAPIKeyDisabled && *req.Status != StatusAPIKeyDisabled {
		return nil, ErrAPIKeyLifecycleRevoked
	}
