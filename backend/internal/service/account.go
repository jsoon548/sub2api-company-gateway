...
func (a *Account) IsSchedulable() bool {
	...
	if a.hasProviderAPIKeyReference() && strings.TrimSpace(a.GetCredential("api_key")) == "" {
		return false
	}
	now := time.Now()
...
func (a *Account) GetCredential(key string) string {
	...
	if a.Credentials != nil {
		value = credentialValueString(a.Credentials[key])
	}
	if key != "api_key" || strings.TrimSpace(value) != "" {
		return value
	}
	return a.resolveProviderAPIKeyReference()
}

func credentialValueString(value any) string {
