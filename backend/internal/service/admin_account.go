...
func (s *adminServiceImpl) CreateAccount(ctx context.Context, input *CreateAccountInput) (*Account, error) {
	...
	if err := validateProviderAPIKeyReference(input.Platform, input.Type, input.Credentials); err != nil {
		return nil, err
	}
...
func (s *adminServiceImpl) UpdateAccount(ctx context.Context, id int64, input *UpdateAccountInput) (*Account, error) {
	...
	if input.Type != "" || len(input.Credentials) > 0 {
		if err := validateProviderAPIKeyReference(account.Platform, account.Type, account.Credentials); err != nil {
			return nil, err
		}
	}
