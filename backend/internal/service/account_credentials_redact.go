...
func MergePreservingSensitiveCreds(existing, incoming map[string]any) map[string]any {
	...
	// api_key_ref is safe to expose to administrators, so it is deliberately
	// not in SensitiveCredentialKeys. It still carries the account's provider
	// authentication identity and must survive an update that omits it, just as
	// an omitted literal API key does.
	if _, hasIncoming := incoming[providerAPIKeyRefCredentialKey]; !hasIncoming {
		if existingVal, ok := existing[providerAPIKeyRefCredentialKey]; ok {
			out[providerAPIKeyRefCredentialKey] = existingVal
		}
	}
	return out
}
