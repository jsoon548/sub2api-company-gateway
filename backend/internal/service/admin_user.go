...
func (s *adminServiceImpl) CreateUser(ctx context.Context, input *CreateUserInput) (*User, error) {
	...
	if role == RoleAdmin {
		if err := s.requireCurrentSuperAdmin(ctx, input.ActorAdminID); err != nil {
			return nil, err
		}
	}
	...
	user := &User{
		...
		Notes:    input.Notes,
		// Elevated identity creation is a two-step fail-closed operation: create
		// a harmless user first, then atomically promote it with governance evidence.
		Role:          RoleUser,
		Balance:       balance,
	...
	if role == RoleAdmin {
		if err := s.governanceRepo.ChangeUserRole(ctx, AdminRoleChangeInput{
			OperationID: uuid.New(), ActorUserID: input.ActorAdminID, TargetUserID: user.ID,
			TargetRole: RoleAdmin, Reason: "create_admin_identity",
		}); err != nil {
			return nil, err
		}
...
func (s *adminServiceImpl) requireCurrentSuperAdmin(ctx context.Context, actorUserID int64) error {
	if s == nil || s.governanceRepo == nil || actorUserID <= 0 {
		return ErrAdminIdentityForbidden
	}
	allowed, err := s.governanceRepo.IsCurrentSuperAdmin(ctx, actorUserID)
	if err != nil {
		return err
	}
	if !allowed {
...
func (s *adminServiceImpl) UpdateUser(ctx context.Context, id int64, input *UpdateUserInput) (*User, error) {
	...
	if input.Status != "" {
		return nil, errors.New("use the explicit deactivate/reactivate operation")
	}
	desiredRole, err := normalizeUserRole(input.Role, user.Role)
	if err != nil {
		return nil, err
	}
	if user.Role == RoleSuperAdmin || user.Role == RoleAdmin || desiredRole == RoleAdmin {
		if err := s.requireCurrentSuperAdmin(ctx, input.ActorAdminID); err != nil {
			return nil, err
		}
	}
	roleChanged := desiredRole != user.Role
	if roleChanged {
		if err := s.governanceRepo.ChangeUserRole(ctx, AdminRoleChangeInput{
			OperationID: uuid.New(), ActorUserID: input.ActorAdminID, TargetUserID: id,
			TargetRole: desiredRole, Reason: "admin_users_update_role",
		}); err != nil {
			return nil, err
		}
		if s.authCacheInvalidator != nil {
...
func (s *adminServiceImpl) DeleteUser(ctx context.Context, id int64) error {
	return ErrPhysicalUserDeletionDisabled
}
...
func (s *adminServiceImpl) BindUserAuthIdentity(ctx context.Context, userID int64, input AdminBindAuthIdentityInput) (*AdminBoundAuthIdentity, error) {
	...
	if target.Role == RoleAdmin || target.Role == RoleSuperAdmin {
		if err := s.requireCurrentSuperAdmin(ctx, input.ActorAdminID); err != nil {
			return nil, err
		}
	}
