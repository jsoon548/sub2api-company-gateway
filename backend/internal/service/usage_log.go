...
type UsageLog struct {
	...
	AccountID int64
	RequestID string
	// GatewayRequestID is the Gateway-owned audit/usage correlation key. It
	// never replaces or derives the legacy/upstream RequestID.
...
func usageGatewayRequestIDFromContext(ctx context.Context) *uuid.UUID {
	if ctx == nil {
		return nil
	}
	id, ok := ctx.Value(ctxkey.GatewayRequestID).(uuid.UUID)
	if !ok || id == uuid.Nil {
		return nil
	}
	value := id
