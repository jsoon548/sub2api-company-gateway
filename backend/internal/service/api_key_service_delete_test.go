...
func TestAPIKeyService_Update_LifecycleDisabledKeyCannotBeReactivated(t *testing.T) {
	repo := &apiKeyRepoStub{apiKey: &APIKey{
		ID:     42,
		UserID: 7,
		Key:    "sk-lifecycle-revoked",
		Name:   "revoked",
	...
	_, err := svc.Update(context.Background(), 42, 7, UpdateAPIKeyRequest{Status: &active})

	require.ErrorIs(t, err, ErrAPIKeyLifecycleRevoked)
	require.Empty(t, repo.updatedKeys, "terminal lifecycle revocation must not reach the repository update")
}

func TestAPIKeyService_Update_LifecycleDisabledKeyCannotMoveThroughInactive(t *testing.T) {
	repo := &apiKeyRepoStub{apiKey: &APIKey{
		ID:     42,
		UserID: 7,
		Key:    "sk-lifecycle-revoked",
	...
	_, err := svc.Update(context.Background(), 42, 7, UpdateAPIKeyRequest{Status: &inactive})

	require.ErrorIs(t, err, ErrAPIKeyLifecycleRevoked)
	require.Empty(t, repo.updatedKeys, "disabled -> inactive -> active must not bypass lifecycle revocation")
}

func TestAPIKeyService_Update_UserInactiveKeyCanBeReactivated(t *testing.T) {
	repo := &apiKeyRepoStub{apiKey: &APIKey{
		ID:     43,
		UserID: 7,
		Key:    "sk-user-inactive",
		Name:   "inactive",
		Status: "inactive",
	}}
	svc := &APIKeyService{apiKeyRepo: repo}
	...
	updated, err := svc.Update(context.Background(), 43, 7, UpdateAPIKeyRequest{Status: &active})

	require.NoError(t, err)
	require.Equal(t, StatusAPIKeyActive, updated.Status)
	require.Len(t, repo.updatedKeys, 1)
	require.Equal(t, StatusAPIKeyActive, repo.updatedKeys[0].Status)
}
