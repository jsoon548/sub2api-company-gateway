...
func createAdminUser(cfg *SetupConfig) (bool, string, error) {
	...
	if err := tx.QueryRowContext(ctx, "SELECT COUNT(1) FROM users WHERE role IN ($1, $2)", service.RoleSuperAdmin, service.RoleAdmin).Scan(&adminUsers); err != nil {
		return false, "", err
	}
	...
	if _, err := tx.ExecContext(ctx, `INSERT INTO super_admin_seat(singleton_id,user_id,version,updated_at) VALUES (1,$1,1,NOW())`, adminID); err != nil {
		return false, "", err
	}
	operationID := uuid.New()
	if _, err := tx.ExecContext(ctx, `INSERT INTO governance_events(id,operation_id,event_sequence,actor_kind,target_kind,target_id,action,result,reason,after_summary,occurred_at) VALUES ($1,$2,1,'system','user',$3,'initialize_super_admin','succeeded','initial_setup',$4,NOW())`, uuid.New(), operationID, strconv.FormatInt(adminID, 10), `{"role":"super_admin","status":"active"}`); err != nil {
		return false, "", err
	}
